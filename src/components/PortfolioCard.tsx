...
export default function PortfolioCard({ portfolio, user, onUpvote, rank }: PortfolioCardProps) {
  ...
  return (
    <div className="w-full h-auto p-3 rounded-lg hover:bg-[#fafafa] hover:dark:bg-slate-900">
      <Link href={`/portfolios/${portfolio.id}`} className="block w-full rounded-lg">
        <div className="w-full rounded-lg aspect-[350/222] overflow-hidden">
          {imageSrc ? (
            ...
            <div className="w-full bg-[#F5F2F0] dark:bg-gray-900  rounded-lg aspect-[350/222]"></div> // Platzhalter
          )}
        </div>
      </Link>
      <div className="w-full h-[68px] mt-1 flex items-center gap-3">
        ...
        <button
          ...
          className="w-11 h-11 bg-white dark:bg-gray-950 border-2 border-[#000000] dark:border-gray-600 rounded-md flex flex-col items-center justify-center gap-0 hover:border-blue-500 dark:hover:border-blue-500"
        >
